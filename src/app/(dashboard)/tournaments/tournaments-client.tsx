...
export default function TournamentsClient({ data }: { data: Tournament[] }) {
  ...
  const megaTournaments = data.filter(t => t.category === 'Mega' || t.prize_pool >= 5000);

  const upcomingTournaments = data.filter(t => {
    if (!t.start_time) return false;
    return new Date(t.start_time).getTime() > now.getTime();
  });
  ...
  return (
    <Tabs defaultValue="upcoming">
      <TabsList>
        ...
        <TabsTrigger value="mega">Mega ({megaTournaments.length})</TabsTrigger>
