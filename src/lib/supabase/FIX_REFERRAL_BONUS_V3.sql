-- FIX REFERRAL BONUS V3 (User Requested Rules)
-- Rules:
-- 1. With Refer Code: 25 Rs.
-- 2. Without Refer Code: 0 Rs.
-- 3. Referrer: Gets 'signup_bonus_referral_referrer' from settings (Default 25).

-- A. UPDATE SETTINGS TO MATCH USER RULES
...
-- B. RECREATE FUNCTION WITH DYNAMIC LOGIC
...
BEGIN
  ...
  SELECT 
    COALESCE((SELECT setting_value::decimal FROM public.app_settings WHERE setting_key = 'signup_bonus_default'), 0),
    COALESCE((SELECT setting_value::decimal FROM public.app_settings WHERE setting_key = 'signup_bonus_referral_user'), 25),
    COALESCE((SELECT setting_value::decimal FROM public.app_settings WHERE setting_key = 'signup_bonus_referral_referrer'), 25)
  INTO 
    bonus_default,
    bonus_referral_user,
  ...
  -- 9. Log New User Transaction (Only if bonus > 0)
...
EXCEPTION WHEN OTHERS THEN
  -- Fallback to ensure signup doesn't fail even if triggers crash
